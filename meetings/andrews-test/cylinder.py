"""
This code does visualization in a 3D cylinderical structure.
The radius of the cylinder depends on the fourier coefficient values.
...
def plot_andrews(lst, gen_bases, bound, clrfunc):
    for i,dataset in enumerate(lst):
        for v in dataset:
                andrews_curve(v, gen_bases, bound, "blue")
...
def main():
    ...
    # plot_andrews([iris_sa, iris_vc, iris_va], sinusoid, [-math.pi, math.pi], clr_by_dist)
    plot_andrews([iris_sa, iris_vc, iris_va], legendre, [-1.0, 1.0], clr_by_dist)
    # plot_andrews([iris_sa, iris_vc, iris_va], triangle, [-math.pi, math.pi], clr_by_dist)
    plt.show()
