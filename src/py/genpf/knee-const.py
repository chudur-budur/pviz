...
def constraint_violation(xvals, fvals):
    ...
    for i,fv in enumerate(fvals):
        c = (math.fsum([((f - 1.75) ** 2) for f in fv[0:-1]]) + ((fv[-1] - 2.1) ** 2)) \
                - (epsilon[m] * epsilon[m])
        if c <= 0:
            cv.append(c)
...
if __name__ == "__main__":
    ...
    utils.cat(fgv)
    outfile = path + "knee-const-{0:d}d.out".format(m)
    utils.save(fv, outfile)
    outfile = path + "knee-const-{0:d}d-cv.out".format(m)
