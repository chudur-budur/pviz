...
def palettize_rv(points, layers, n_layers = 0, zgap = 1.0, inverted = False):
    """
    ...
    using f_i = f_i^p where p = 2.0 if m > 3 else 1.0. If 'inverted' is
    True, then it will invert the radviz mapping.
    """
    points_ = copy.copy(points)
    m = len(points_[0])
    if inverted:
        for i in range(len(points_)):
            points_[i] = [(1.0 - v) for v in points_[i]]
...
def palettize_sc(points, layers, n_layers = 0, zgap = 1.0, inverted = False):
    """
...
if __name__ == "__main__":
    ...
    if mode == "invsc":
        ...
        palettefpath = os.path.join(path, normfile.split('.')[0] + "-palette-invsc.out")
    elif mode == "invrv":
        palette_coords = palettize_star(points, layers, n_layers = n_layers)
        palettefpath = os.path.join(path, normfile.split('.')[0] + "-palette-invrv.out")
    else:
